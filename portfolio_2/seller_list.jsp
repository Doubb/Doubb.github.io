...
<html>
    ...
    <script>
        function product_register() {
            let url = "product_page_seller.jsp?id=<%=request.getParameter("id")%>";

            window.open(url);
        }

        function product_modify() {
            let url = "product_modify_seller.jsp?id=<%=request.getParameter("id")%>";
    ...
    <h2>Seller List</h2>
    <div id="buttons">
    <button onclick="product_register()">Product Registration</button> <button onclick="product_modify()">Product Modification</button>
    </div>
...
<%
    try{
        ...
        ResultSet rs = st.executeQuery("SELECT * FROM new_schema.product");
        ...
        <tr>
            ...
            <td><%=rs.getString(8)%> <br> <%=rs.getString(9)%> <br> <%=rs.getString(10)%></td>
        </tr>
